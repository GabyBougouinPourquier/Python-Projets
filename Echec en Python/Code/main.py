"""Voici mon super projet ! un jeu d'echec en python ! Il faut savoir que j'ai utiliser chatgpt pour resoudre un
probleme (detection des pions touchés a l'endroit ou la souris a clickée.
Je viens d'avoir 13 ans donc merci de respecter la (non-logique) du code :)"""
import pygame
...
def deplacer_tour(col, ligne):
    if col == pion_select.x // cell_size or ligne == pion_select.y // cell_size:
        if col * cell_size != pion_select.x or ligne * cell_size != pion_select.y:
            direction = None
            pion_detect = None
            if ligne * cell_size == pion_select.y:
                pos_tour = "horizontal"

                if col * cell_size > pion_select.x:
                    direction = "droite"
                    for droite in range(pion_select.x + cell_size, col * cell_size, cell_size):
                        for pion in pions_noirs:
                            if pion.y == ligne * cell_size and pion.x == droite:
                                print("Bloqué en x =", droite)
                                pion_detect = True
                                break

                            
                elif col * cell_size < pion_select.x:
                    direction = "gauche"
                else:
                    direction = "erreur"
            elif col * cell_size == pion_select.x:
                ...
                elif ligne * cell_size < pion_select.y:
                    direction = "haut"
                else:
                    direction = "erreur"
            else:
                pos_tour = None
            print(pos_tour, direction, pion_detect)
